...
namespace Neoan\Framework\Database\Transformers;
...
class Transform
{
     ...
     */
    public function __construct($model, Database $db, $modelStructure = null)
    {
     ...
     * @return \Generator
     */
    public function getGenerator($ids) : Generator
    {
        foreach ($ids as $id) {
     ...
     * @throws \Exception
     */
    public function create($inserts) : array
    {
        ...
        $main['id'] = $id;
        $t = $this->db->smart($this->modelName, $this->validate($this->modelName, $main));
     ...
     * @throws \Exception
     */
    public function find(array $condition = [], array $callFunctions = []) : array
    {
        $joinTables = [];
            ...
            if (isset($this->modelStructure[$table][$field])) {
                ...
                switch ($this->cleanType($this->modelStructure[$table][$field]['type'])) {
                    ...
                    case 'datetime':
                        ...
                        $returnArray[$field] = !empty($date_array['errors']) ? null : date('Y-m-d H:i:s',
                            mktime($date_array['hour'], $date_array['minute'], $date_array['second'], $date_array['month'], $date_array['day'], $date_array['year']));
                        break;
                    case 'decimal':
